#' Compute sigma_u, sigma_e, and rho after fitting a fixed or random effects
#' model with \pkg{plm}
#'
#' @export
#' @param fit A model fit with \code{\link[plm]{plm}} and \code{model =
#'   "within"} or \code{model = "random"}.
#' @return A numeric vector containing the named components 
#' \itemize{ 
...
sigmaRho <- function(fit){
  ...
  if (model == "within") {
    sigma_u <- sd(plm::fixef(fit))
    
    rfit <- update(fit, model = "random")
    ercomp <- plm::ercomp(rfit)[["sigma2"]]
    sigma_e <- sqrt(ercomp$idios)
  } else {
    ercomp <- plm::ercomp(fit)[["sigma2"]]
